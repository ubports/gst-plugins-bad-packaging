...
build-binary.sh'''
              stash(includes: '*.gz,*.bz2,*.xz,*.deb,*.ddeb,*.debb,*.dsc,*.changes,*.buildinfo,lintian.txt', name: 'build-armhf')
              cleanWs(cleanWhenAborted: true, cleanWhenFailure: true, cleanWhenNotBuilt: true, cleanWhenSuccess: true, cleanWhenUnstable: true, deleteDirs: true)
            }
 ...
 #   build-binary.sh'''
    ...
    stage('Results') {
      steps {
        ...
        unstash 'build-arm64'
        unstash 'build-amd64'
        archiveArtifacts(artifacts: '*.gz,*.bz2,*.xz,*.deb,*.ddeb,*.debb,*.dsc,*.changes,*.buildinfo', fingerprint: true, onlyIfSuccessful: true)
